...
def hinder
  ...
  # Create objects
  # if there is a bunker, remove it
  if $bunker1
    hide1
  end
...
def collides_with_obstacle?(obstacle)
...
def golf1
  ...
  if $ball_in_motion
    ...
    $obstacles.each do |obstacle|
      if collides_with_obstacle?(obstacle)
    ...
    if in_bunker?($bunker1) || in_bunker?($bunker2)
...
on :key_down do |event|
  reset_ball if event.key == 'r' && !$ball_in_motion
end
